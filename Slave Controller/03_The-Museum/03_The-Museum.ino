...
String html_10 = "<form id='F2' action='P2_X_ON'><input class='button' type='submit' value='P2 EEG ON' ></form><br>";
String html_11 = "<form id='F2' action='P2_X_OFF'><input class='button' type='submit' value='P2 EEG OFF' ></form><br>";

String html_12 = "<form id='F2' action='P3_X_ON'><input class='button' type='submit' value='P3 EMP ON' ></form><br>";
String html_13 = "<form id='F2' action='P3_X_OFF'><input class='button' type='submit' value='P3 EMP OFF' ></form><br>";
...
void P2_light_ON() {
  digitalWrite(4, HIGH);
  Serial.println("P2_light_ON");
}

void P2_light_OFF() {
  digitalWrite(4, LOW);
  Serial.println("P2_light_OFF");
}

void P3_light_ON() {
  digitalWrite(0, HIGH);
  Serial.println("P3_light_ON");
}

void P3_light_OFF() {
  digitalWrite(0, LOW);
  Serial.println("P3_light_OFF");
}
...
void P1_Motor_ON() {
  digitalWrite(2, HIGH);
  Serial.println("P1_Motor_ON");
}

void P1_Motor_OFF() {
  digitalWrite(2, LOW);
  Serial.println("P1_Motor_OFF");
}

void P2_X_ON() {
  digitalWrite(14, HIGH);
  Serial.println("P2_X_ON");
}

void P2_X_OFF() {
  digitalWrite(14, LOW);
  Serial.println("P2_X_OFF");
}

void P3_X_ON() {
  digitalWrite(12, HIGH);
  Serial.println("P3_X_ON");
}

void P3_X_OFF() {
  digitalWrite(12, LOW);
  Serial.println("P3_X_OFF");
}
...
void loop() {
  ...
  else if  ( request.indexOf("P2_ON") > 0 ) { P2_light_ON();   }
  else if  ( request.indexOf("P2_OFF") > 0 ) { P2_light_OFF();   }
  else if  ( request.indexOf("P3_ON") > 0 ) { P3_light_ON();   }
